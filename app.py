...
###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
# T ODO 364: Check out the included file giphy_api_key.py and follow the instructions in TODOs there before proceeding to view functions.

###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
# T ODO 364: All templates you need are provided and should not be edited. However, you will need to inspect the templates that exist in order to make sure you send them the right data!
...
###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:arnoldzhoumi14@localhost/ahzhouHW4db" # T ODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
# N OTE - 364: You may want to complete the models tasks below BEFORE returning to build the association tables! That will making doing this much easier.
###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
# N OTE: Remember that setting up association tables in this course always has the same structure! Just make sure you refer to the correct tables and columns!
###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
# T ODO 364: Set up association Table between search terms and GIFs (you can call it anything you want, we suggest 'tags' or 'search_gifs').
search_gifs = db.Table('search_gifs', db.Column('searchterm_id', db.Integer, db.ForeignKey('searchterm.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gif.id')))
###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
# T ODO 364: Set up association Table between GIFs and collections prepared by user (you can call it anything you want. We suggest: user_collection)
user_collection = db.Table('user_collection', db.Column('personalgifcollection_id', db.Integer, db.ForeignKey('personalgif.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gif.id')))
...
class User(UserMixin, db.Model):
    ...
    #T ODO 364: In order to complete a relationship with a table that is detailed below (a one-to-many relationship for users and gif collections), you'll need to add a field to this User model. (Check out the TODOs for models below for more!)
    # Remember, the best way to do so is to add the field, save your code, and then create and run a migration!
    usergif = db.relationship('PersonalGifCollection', backref='User')#building the relationship: one user, many personalgifcollections
...
# T ODO 364: Read through all the models tasks before beginning them so you have an understanding of what the database structure should be like. Consider thinking about it as a whole and drawing it out before you write this code.

# Model to store gifs
class Gif(db.Model):
    __tablename__ = "gif"
    ###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
    # T ODO 364: Add code for the Gif model such that it has the following fields:
    ...
    # T ODO 364: Define a __repr__ method for the Gif model that shows the title and the URL of the gif
    def __repr__(self):
        return "{%s}: {%s}" % (self.title, self.embedURL)

# Model to store a personal gif collection
class PersonalGifCollection(db.Model):
    __tablename__ = "personalgif"
    ###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
    # T ODO 364: Add code for the PersonalGifCollection model such that it has the following fields:
    # id (Integer, primary key)
    ...
    ###Has a one-to-many relationship through User.usergif.
    ...
    gifs = db.relationship('Gif', secondary=user_collection, backref=db.backref('personalgif',lazy='dynamic'), lazy='dynamic')
...
class SearchTerm(db.Model):
    ...
    # T ODO 364: Add code for the SearchTerm model such that it has the following fields:
    # id (Integer, primary key)
    ...
    # T ODO 364: Define a __repr__ method for this model class that returns the term string
    def __repr__(self):
        return "{%s}" % (self.term)
...
###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
# T ODO 364: The following forms for searching for gifs and creating collections are provided and should not be edited. You SHOULD examine them so you understand what data they pass along and can investigate as you build your view functions in TODOs below.
...
def get_gifs_from_giphy(search_string):
    ...
    ###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
    # T ODO 364: This function should make a request to the Giphy API using the input search_string, and your api_key (imported at the top of this file)
    ...
    apiurl = baseurl + "?q=" + str(search_string) + "&api_key=" + str(api_key) + "&limit=5"
...
def get_or_create_gif(title, url):
    ...
    # T ODO 364: This function should get or create a Gif instance. Determining whether the gif already exists in the database should be based on the gif's title.
...
def get_or_create_search_term(term):
    ...
    # T ODO 364: This function should return the search term instance if it already exists.
    ...
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then append the return value from get_or_create_gif to the search term's associated gifs (remember, many-to-many relationship between search terms and gifs, allowing you to do this!).
        for gif in gif_list:
            gif = get_or_create_gif(gif["title"], gif["embed_url"])
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    ###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
    # T ODO 364: This function should get or create a personal gif collection. Uniqueness of the gif collection should be determined by the name of the collection and the id of the logged in user.
    current_user_id = User.query.filter_by(username=current_user).first().id
    personalcollection = PersonalGifCollection.query.filter_by(name=name, user_id=current_user_id).first()
    ...
    if personalcollection is not None:
    ...
    personalcollection = PersonalGifCollection(name=name,user_id=current_user_id)
    for gif in gif_list:
        personalcollection.gifs.append(gif)
    db.session.add(personalcollection)
    db.session.commit()
    return personalcollection
...
def index():
    ###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
    # T ODO 364: Edit this view function, which has a provided return statement, so that the GifSearchForm can be rendered.
...
def search_results(search_term):
    ...
    relevant_gifs = []#Much better implementation of using relevant_gifs
    if term:
        relevant_gifs = term.gifs.all()
...
def search_terms():
    ###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
    # T ODO 364: Edit this view function so it renders search_terms.html.
...
def create_collection():
    ...
    if form.validate_on_submit():
        name = form.name.data
        selected_list = []
        for gif in form.gif_picks.data:
            selected_list.append(get_gif_by_id(gif))
        get_or_create_collection(name, current_user.username, selected_list)
...
def collections():
    ###DONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONEDONE
    # T ODO 364: This view function should render the collections.html template so that only the current user's personal gif collection links will render in that template. Make sure to examine the template so that you send it the correct data!
    user_id = User.query.filter_by(username=current_user.username).first().id
    collections = PersonalGifCollection.query.filter_by(user_id=user_id).all()
